...
function Butterlation_chklng(lng) {
    if (lng.indexOf("zh") != -1) {
        if (lng.indexOf("cn") != -1 || lng.indexOf("CN") != -1)
            return "zhcn";
        else
            return "zhtw";
    }
...
else if (lng.indexOf("jp") != -1) {
    return "jp";
}
else if (lng.indexOf("ar") != -1) {
    return "ar";
}
else if (lng.indexOf("ko") != -1 || lng.indexOf("korean") != -1) {
    return "ko";
}
    return "en";
}

function Butterlation_getlng() {
    var lng = "en";
    ...
    return Butterlation_chklng(lng);
}
...
function Butterlation() {
  ...
  this.setLang = function (lg) {
      this.lang = Butterlation_chklng(lg);
  }
...
function  isIE(){
	...
	if  (userAgent.indexOf("msie")!=-1){
		...
		var i;
		try{
			i = (!window.ActiveXObject && !!window.ActiveXObject.toString)
		}catch(err){
			return  false; 
		}
		if(i) return true;
